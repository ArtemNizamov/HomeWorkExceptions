    ...
    public void checkLogin(String login) throws WrongLogicException {
        for (int i = 0; i < login.length(); i++) {
            if (VALID_SYMBOLS.contains(String.valueOf(login.charAt(i)))) {
                throw new WrongLogicException("Логин содержит запрещенные символы" + login.charAt(i));
            }
        }
    ...
    public void checkPassword(String password) throws WrongPasswordException {
        ...
        for (int i = 0; i < password.length(); i++) {
            if (VALID_SYMBOLS.contains(String.valueOf(password.charAt(i)))) {
                throw new WrongPasswordException("Пароль содержит запрещенные символы" + password.charAt(i));
            }
